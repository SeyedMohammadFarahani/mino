...
@section('content')
    ...
    <div class="container mt-5" style="width: 80%">
        ...
        <form action="{{ route('processes.destroyAll') }}" method="post">
            @csrf

            <a href="{{route('processes.create')}}" class="btn btn-success justify-content-md-end"
                ...
                <table class="table table-bordered table-striped">
                    ...
                    @foreach($processes as $process)
                        ...
                        <tr id="tr_{{$process->id}}">
                            <td><input type="checkbox" name="processes[]" value="{{$process->id}}"></td>
                            <th scope="row"> <?php echo $i;?> </th>
                            <td>{{ $process->name }}</td>
                            <td>{{ $process->material }}</td>
                            <td>{{ number_format($process->price,4,'.',',') }}</td>
                            <td>{{ number_format($process->salary,4,'.',',') }}</td>
                            <td><i class="icon-trash"></i>
                                <a class="btn btn-danger"
                                   href="{{ route('processes.destroy', ['id' => $process->id])}}">حذف</a>
                            </td>
                            <td><i class="icon-trash"></i>
                                <a class="btn btn-warning" href="{{ route('processes.edit', ['id' => $process->id]) }}">ویرایش</a>
                            </td>
                        </tr>
                        <?php $i++;?>
                    @endforeach
